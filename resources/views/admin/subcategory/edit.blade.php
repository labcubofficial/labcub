...
<div class="bg-indigo-600 px-8 pt-10 lg:pt-14 pb-16 flex justify-between items-center mb-3">
    <h1 class="text-xl text-white">Edit Sub Category</h1>
</div>
<div class="-mt-12 mx-6 mb-6 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-12 xl:grid-cols-12">
    <div class="card h-full shadow">
        <div class="border-b border-gray-300 px-5 py-4 flex items-center w-full justify-between">
            <form class="w-full" action="{{ route('subcategory.update', $sub_category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!-- input -->
                ...
                <div class="mb-6 inline-flex md:flex md:items-center gap-3 flex-col md:flex-row w-full">
                    ...
                    <div class="flex-[3] w-full">
                        <x-input name="category_name" placeholder="Enter Category Name" value="{{ $sub_category->category_name }}" />
                    </div>
                </div>
                    ...
                    <div class="flex-[3] w-full">
                        <select name="parent_category" class="py-2 px-4 pe-9 block w-full border-gray-300 rounded focus:border-indigo-600 focus:ring-indigo-600 disabled:opacity-50 disabled:pointer-events-none">
                            @foreach($subcategory as $item)
                                <option value="{{ $item->id }}" {{ ($sub_category->parent_id == $item->id)?'selected':'' }}>{{ $item->category_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                            ...
                            <div class="text-center">
                                @if(isset($sub_category->image))
                                    <div class="preview-image">
                                        <img src="{{ url('media/sub_category').'/'.$sub_category->image }}" alt="" class="previewImage" style="width: 100px; height: 100px;">
                                    </div>
                                @else
